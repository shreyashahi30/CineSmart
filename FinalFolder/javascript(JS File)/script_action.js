...
// Function to fetch and display popular action movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const actionMoviesContainer = document.getElementById('movies');
            actionMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Action" genre (ID: 28)
            const actionGenreId = 28; // Action genre ID from TMDB
