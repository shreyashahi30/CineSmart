...
// Function to fetch and display popular sci-fi movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const sciFiMoviesContainer = document.getElementById('movies');
            sciFiMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Science Fiction" genre (ID: 878)
            const sciFiGenreId = 878; // Sci-Fi genre ID from TMDB
            const sciFiMovies = movies.filter(movie => 
                movie.genre_ids.includes(sciFiGenreId)
            );
...
function getGenreName(genreId) {
    ...
    return genreMapping[genreId]; // No default return for unknown genres
}
