...
// Function to fetch and display popular fantasy movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const fantasyMoviesContainer = document.getElementById('movies');
            fantasyMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Fantasy" genre (ID: 14)
            const fantasyGenreId = 14; // Fantasy genre ID from TMDB
            const fantasyMovies = movies.filter(movie => 
                movie.genre_ids.includes(fantasyGenreId)
            );
            ...
            fantasyMovies.forEach(movie => {
                ...
                fantasyMoviesContainer.appendChild(movieCard);
