...
// Function to fetch and display popular drama movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const dramaMoviesContainer = document.getElementById('movies');
            dramaMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Drama" genre (ID: 18)
            const dramaGenreId = 18; // Drama genre ID from TMDB
            const dramaMovies = movies.filter(movie => 
                movie.genre_ids.includes(dramaGenreId)
            );
...
function getGenreName(genreId) {
    ...
    return genreMapping[genreId]; // Removed 'Unknown Genre' return
}
