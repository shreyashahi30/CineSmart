...
// Function to fetch and display popular horror movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const horrorMoviesContainer = document.getElementById('movies');
            horrorMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Horror" genre (ID: 27)
            const horrorGenreId = 27; // Horror genre ID from TMDB
            const horrorMovies = movies.filter(movie => 
                movie.genre_ids.includes(horrorGenreId)
            );
...
function getGenreName(genreId) {
    ...
    return genreMapping[genreId]; // Removed 'Unknown Genre' fallback
}
