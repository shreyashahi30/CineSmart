...
// Function to fetch and display popular animation movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const animationMoviesContainer = document.getElementById('movies');
            animationMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Animation" genre (ID: 16)
            const animationGenreId = 16; // Animation genre ID from TMDB
            const animationMovies = movies.filter(movie => 
                movie.genre_ids.includes(animationGenreId)
            );
            ...
            animationMovies.forEach(movie => {
                ...
                animationMoviesContainer.appendChild(movieCard);
...
function getGenreName(genreId) {
    ...
    return genreMapping[genreId]; // Removed 'Unknown Genre'
}
