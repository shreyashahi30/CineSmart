...
// Function to fetch and display popular romance movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const romanceMoviesContainer = document.getElementById('movies');
            romanceMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Romance" genre (ID: 10749)
            const romanceGenreId = 10749; // Romance genre ID from TMDB
            const romanceMovies = movies.filter(movie => 
                movie.genre_ids.includes(romanceGenreId)
            );
            ...
            romanceMovies.forEach(movie => {
                ...
                romanceMoviesContainer.appendChild(movieCard);
