...
// Function to fetch and display popular comedy movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const comedyMoviesContainer = document.getElementById('movies');
            comedyMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Comedy" genre (ID: 35)
            const comedyGenreId = 35; // Comedy genre ID from TMDB
