...
// Function to fetch and display popular adventure movies
function fetchPopularMovies() {
    fetch(popularMoviesUrl)
        .then(response => response.json())
        .then(data => {
            const movies = data.results;
            const adventureMoviesContainer = document.getElementById('movies');
            adventureMoviesContainer.innerHTML = ''; // Clear any existing content

            // Filter movies by "Adventure" genre (ID: 12)
            const adventureGenreId = 12; // Adventure genre ID from TMDB
            ...
            adventureMovies.forEach(movie => {
                ...
                adventureMoviesContainer.appendChild(movieCard);
